...
export default function Shippersignup(props) {
  ...
  const handleSubmit = (shipperInfo, event) => {
    ...
    if (sinputFormValidation(shipperInfo)) {
      axios.post('/users', {user: shipperInfo})
        .then(userinfo => {
...
{sstate.view === 'shipper' && 
      ...
      <div className="userform">
        ...
        <form onSubmit={event =>  handleSubmit(sstate.shipperInfo, event)}>
          ...
          <input type="text" name='name' value={'value'} onChange={(e) => sinfo_handler('name', e.target.value)}/>
          </p>
          ...
          <p>
            ...
            <input type="url" name='web_link' id="web_link" value={'text'} onChange={(e) => sinfo_handler('web_link', e.target.value)} placeholder="www.example.ca"/>
          </p>
          ...
          <p>
            ...
            <input type="button" name='ccard_info'  id="ccard_info" onClick={sv_handler('ccard_info')} />
          </p>
          
          <p>
            <label htmlFor="company_infomation">Company_infomation:</label>
            <input type="button" name='company_infomation' onClick={sv_handler('company_infomation')} id="company_infomation" />
          </p>
          
          <p>
            <label htmlFor="driving_record">Driving_record:</label>
            <input type="button" name='driving_record' onClick={sv_handler('company_infomation')} id="company_infomation" />
          </p>

          <p>
            <label htmlFor="photo_id">PhotoID:</label>
            <input type="file" name='photo_id' id="photo_id" onClick={sv_handler('photo_id')} placeholder="uploade one file of both faces. Clear copies!"/>
          </p>       
          
          <p>
            <label htmlFor="work_schedule">Work_schedule:</label>
            <input type="button" name='work_schedule' id="work_schedule"  onClick={sv_handler('work_schedule')} placeholder="click to enter your swork schedule. Optional!"/>
          </p>
